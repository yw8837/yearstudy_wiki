...
> 강의 출처: [[0605-생성형AI기초]] · 상세: [[생성형AI]] · [[LLM]] · [[AIAgent]] · [[PhysicalAI]] · [[멀티모달AI]] · [[Diffusion모델]] · ★[[심볼릭AI]] · [[RAG]]
...
## 9. ★ 심볼릭 AI — 모델 + 지식 → [[심볼릭AI]]

```
사용자 → 질문 → [ AI 시스템 = 모델 + 지식 ] → 답변
아키텍처는 안 바뀜. 바뀌는 건 둘 — ① 모델(선택만) ② 지식(업로드, 품질 결정)
```

- 1960~70년대 개념이 현재 ChatGPT·Gemini·Claude에 그대로 차용 (강사 핵심).
- 서비스 설계 함의: 모델 선택권 + 지식 업로드 공간을 제공해야 한다 (예: Genspark 드라이브).
- **일반 채팅 vs 지식 업로드** 답변 품질 차이가 큼 — 업로드 쪽이 상세·출처(페이지)까지 제시.

## 10. ★ RAG — 환각 줄이는 지식 주입 → [[RAG]]

```
질문 → [관련 문서 검색] → 문서 + 질문을 모델에 입력 → 근거 기반 답변
```

- "모델+지식"의 **지식** 을 실시간으로 붙이는 엔지니어링.
- [[Hallucination]] 완화책이지만 **0으로는 못 만든다** — 중요 문서는 직접 검증.
- 심화 검색 팁: **"RAG + 강화학습"** ([[RLHF]]).
...
- ★ [[심볼릭AI]] · [[RAG]] · [[Cursor]] — 라이브 신규
- [[0605-생성형AI기초]] — 원본 강의 노트
- [[AI윤리가이드정리]] · [[문서논리정리]] — 같은 날 다른 편
- [[AI]] · [[딥러닝]] — 상위·기반 개념
